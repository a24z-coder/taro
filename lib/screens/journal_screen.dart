...
class _ReflectionSummaryScreenState extends State<ReflectionSummaryScreen> {
  ...
  Future<void> _getAIRecommendations() async {
    ...
    try {
      // Собираем все размышления пользователя с нумерацией
      final allReflections = _reflectionEntries
          .where((entry) => entry?.reflectionText != null && entry!.reflectionText!.isNotEmpty)
          .map((entry) => 'Расклад ${entry!.spreadTitle ?? entry.spreadType}: ${entry.reflectionText}')
          .join('\n\n');

      // Собираем все карты из всех раскладов с переводом на русский
      final allCards = <String>[];
      for (final entry in _reflectionEntries) {
        if (entry != null && entry.cards.isNotEmpty) {
      ...
      // Берём только 1-2 самые частые карты
      final topCards = sortedCards.take(2).map((e) => e.key).toList();
      final cardsString = topCards.join(', ');

      // Получаем имя пользователя
      await UserService().loadUserName();
      final userName = UserService().userName.isNotEmpty ? UserService().userName : AppLocalizations.of(context)!.the_user;

      final loc = AppLocalizations.of(context)!;
      final prompt = loc.reflection_summary_ai_prompt(
        allReflections,
        cardsString,
        userName,
      );
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, i) {
                  if (i == 0) {
                    return Column(
                      children: [
                        // const SizedBox(height: 24), // УБРАН верхний отступ, SafeArea уже есть
        ...
        child: BackdropFilter(
          ...
          child: Container(
            decoration: BoxDecoration(
              color: Colors.white.withOpacity(isFilled ? 0.10 : 0.06), // ещё более прозрачный
              borderRadius: BorderRadius.circular(24),
